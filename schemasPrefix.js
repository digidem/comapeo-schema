const schemasPrefix = {
  common: {schemaVersions: [1]},
  observation: { dataTypeId: '70f85084cb94', schemaVersions: [5] },
  preset: { dataTypeId: '71f85084cb98', schemaVersions: [2] },
  field: { dataTypeId: '71f85084cb80', schemaVersions: [2] },
  coreOwnership: { dataTypeId: '73f85084cb80', schemaVersions: [1] },
  device: { dataTypeId: '13f85084cb80', schemaVersions: [1] },
  role: { dataTypeId: '13fa5384cb80', schemaVersions: [1] },
}
export default schemasPrefix
