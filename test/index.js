...
test(`testing encoding of doc with minimal required values,
  then decoding and comparing the two objects`,
  async(t) => {
    for(let {doc,expected} of fixtures.goodDocsMinimal){
      ...
      t.deepEqual(decodedDoc,expected, `tested deep equal of ${doc.schemaName} with only required values`)
    }
})
...
test(`testing encoding of doc with additional optional values,
  ...
  async(t) => {
    for(let {doc,expected} of fixtures.goodDocsCompleted){
      ...
      let decodedDoc = stripUndef(decode(buf, parseVersionId(doc.versionId)))
      t.deepEqual(decodedDoc,expected, `tested deep equal of ${doc.schemaName} with additional values`)
    }
})

test(`testing encoding of doc with additional extra values,
then decoding and comparing the two objects - extra values shouldn't be present`,
  async(t) => {
    for(let {doc,expected} of fixtures.goodDocsWithExtraFields){
      ...
      let decodedDoc = stripUndef(decode(buf, parseVersionId(doc.versionId)))
      t.deepEqual(decodedDoc,expected, `tested deep equal of ${doc.schemaName} with extra - non valid - values`)
    }
})
